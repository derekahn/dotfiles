...
Clone `dotfiles` into your directory of choice and run the install script. This script will install [Homebrew](https://brew.sh/) (if it needs to) and then install all of dotfiles' dependencies.

**Warning: This will move existing `zsh`, `tmux`, or `nvim` configurations to a backup folder inside of the installation repo.**

```bash
git clone https://github.com/derekahn/dotfiles ~/.dotfiles
cd !$
./install.sh
```
...
5. (Optional) Install [Pecan](https://github.com/zzzeyez/Pecan) for customizable OSX header. Follow install instructions on page. For Oceanic theme, copy `dotfiles/config/Pecan/style.css` into `~/Library/Application Support/Übersicht/widgets/Pecan/style.css` and refresh widget by using `Übersicht` -> `Refresh All Widgets` from native OSX menu bar.
...
These steps currently just detail instructions for getting Neovim working in Windows with `dotfiles`.
...
The following configurations require manual changes outside of `dotfiles`.
...
1. Load settings from folder: `Preferences` -> `General` -> `Preferences` -> `Load preferences from a custom folder or URL` and choose `<your-dotfiles-directory>/iterm`
